  ...
  <div class="home min-h-screen bg-black">
    ...
    >
      <div class="text-center">
        ...
        ></div>
        <p class="text-white text-lg">Loading TV series...</p>
      </div>
    </div>
    ...
    <!-- Main Content -->
    <div v-else class="container mx-auto px-4 py-8 pt-24">
      <div class="search-section animate-fade-in-up mb-8">
        <h2 class="text-white text-3xl lg:text-4xl font-bold mb-6 text-center">
          Search TV Series
        </h2>
        <div class="flex justify-center mb-8">
          <div class="relative w-full max-w-md lg:max-w-lg">
            <input
              ...
              id="searchInput"
              type="text"
              class="w-full h-12 pl-12 pr-4 rounded-full border-2 border-gray-600 bg-gray-800 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all duration-300"
        ...
        <!-- Search Results -->
        <div v-if="searchQuery && searchResults.series.length > 0" class="mb-8">
          <h3 class="text-white text-2xl font-semibold mb-4">
            Search Results for "{{ searchQuery }}"
          </h3>
          <Serieslist :serial="searchResults.series" class="animate-stagger" />
        </div>

        <!-- No Results -->
        <div
          v-else-if="searchQuery && searchResults.series.length === 0"
        ...
        >
          ...
          <p class="text-lg">No TV series found for "{{ searchQuery }}"</p>
          <p class="text-sm mt-2">Try searching for a different TV series</p>
        </div>
      </div>
      <h2
        class="text-white text-3xl lg:text-4xl font-bold mb-6 text-center animate-slide-in-right"
      >
        Popular TV Series
      </h2>
...
const search = async () => {
  ...
  searchResults.value = { series: searchSeries };
};
...
const retryLoad = async () => {
  ...
  try {
    ...
    error.value = "Failed to load TV series. Please refresh the page.";
  } finally {
